...
def _pull_path(context, from_task: str) -> str:
    ...
    return context["task_instance"].xcom_pull(
        task_ids=from_task, key=_XCOM_FILE_KEY
    )
...
def task_write(**context) -> dict[str, str]:
    ...
    df            = pd.read_parquet(tmp)
    src_path      = context["task_instance"].xcom_pull(task_ids="sense_file", key=_XCOM_FILE_KEY)
    original_rows = context["task_instance"].xcom_pull(task_ids="process", key="original_rows")
    rej_path      = context["task_instance"].xcom_pull(task_ids="process", key="rejected_path")
...
def task_archive(**context) -> str:
    # move the source file to processed/ so the sensor picks up the next file next run
    src_path = context["task_instance"].xcom_pull(task_ids="sense_file", key=_XCOM_FILE_KEY)
